...
class _Reports extends \IPS\Content\Controller
{
	...
	public function manage()
	{
		...
		$table->filters = [
			'report_status_1' => array( 'status=1' )
		];
		...
		foreach( $apps as $appKey => $data )
		{
			...
			$table->filters['report_status_1_' . $appKey ] = [ 'status=1 and ' . \IPS\Db::i()->in( 'class', $classes ) ];
			\IPS\Member::loggedIn()->language()->words[ 'report_status_1_' . $appKey ] = \IPS\Member::loggedIn()->language()->addToStack( 'report_status_1_app', NULL, [ 'sprintf' => [ \IPS\Application::applications()[ $appKey ]->_title ] ] );
		}

		$table->filters['report_status_2'] = [ 'status=2' ];
		$table->filters['report_status_3'] = [ 'status=3' ];
