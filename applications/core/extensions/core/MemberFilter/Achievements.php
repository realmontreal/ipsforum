...
class _Achievements
{
	...
	public function getSettingField( $criteria )
	{
		...
		{
			$return[] = new \IPS\Helpers\Form\YesNo( 'mf_badges_choose', ( isset( $criteria['achievement_badges'] ) AND $criteria['achievement_badges'] != 'ignore' ), FALSE, array(
				'togglesOn' => [ 'mf_badges' ]
			), NULL, NULL, NULL, 'mf_badges_choose' );
			$return[] = new \IPS\Helpers\Form\CheckboxSet( 'mf_badges', ( isset( $criteria['achievement_badges'] ) AND $criteria['achievement_badges'] != 'ignore' ) ? explode( ',', $criteria['achievement_badges'] ) : [], FALSE, array(
		...
		{
			$return[] = new \IPS\Helpers\Form\YesNo( 'mf_ranks_choose', ( isset( $criteria['achievement_ranks'] ) AND $criteria['achievement_ranks'] != 'ignore' ), FALSE, array(
				'togglesOn' => [ 'mf_ranks' ]
			), NULL, NULL, NULL, 'mf_ranks_choose' );
			$return[] = new \IPS\Helpers\Form\CheckboxSet( 'mf_ranks', ( isset( $criteria['achievement_ranks'] ) AND $criteria['achievement_ranks'] != 'ignore' ) ? explode( ',', $criteria['achievement_ranks'] ) : [], FALSE, array(
	...
	public function save( $post )
	{
		return array(
			...
			'achievement_badges' => ( isset( $post['mf_badges_choose'] ) AND $post['mf_badges_choose'] ) ? implode( ',', $post['mf_badges'] ) : 'ignore',
			'achievement_ranks' =>  ( isset( $post['mf_ranks_choose'] ) AND $post['mf_ranks_choose'] ) ? implode( ',', $post['mf_ranks'] ) : 'ignore'
		);
	}
