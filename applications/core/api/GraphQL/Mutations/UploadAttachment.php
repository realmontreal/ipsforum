...
class _UploadAttachment
{
	...
	public function resolve($val, $args, $context, $info)
	{
		...
		if( $allowedFileTypes = \IPS\Helpers\Form\Editor::allowedFileExtensions() )
		{
			...
			{
				throw new \IPS\Api\GraphQL\SafeException( 'FILE_TYPE_NOT_ALLOWED', '2C399/1', 403 );
			}
		}
		...
		if ( $storageClass::$supportsChunking and isset( $args['chunk'] ) )
		{
			...
			if ( $args['chunk'] === $args['totalChunks'] )
			{
				...
				$file = \IPS\File::get( 'core_Attachment', $url );
				...
				if ( \in_array( $ext, \IPS\Image::supportedExtensions() ) )
				{
					...
					{
						throw new \IPS\Api\GraphQL\SafeException( 'NOT_VALID_IMAGE', '2C399/2', 403 );
					}
				}
			}
			else
		...
		else
		{
			...
			if ( \in_array( $ext, \IPS\Image::supportedExtensions() ) )
			{
				...
				{
					throw new \IPS\Api\GraphQL\SafeException( $e->getMessage(), '2C399/2', 403 );
				}
			}
			...
			{
				throw new \IPS\Api\GraphQL\SafeException( 'FILE_CREATION_FAILED', '2C399/3', 403 );
			}
		}
