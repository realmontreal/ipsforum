...
HTML,
	...
	public function filterProperties( array $properties )
	{
		...
		foreach ( $this->propertiesConfiguration as $key => $property )
		{
			...
			)
			{
				...
				if ( $member->member_id AND !$member->datalayer_collect_pii )
				{
	...
	protected function get_jsContext()
	{
		...
		if ( \IPS\Settings::i()->core_datalayer_include_pii AND ( !\IPS\Settings::i()->core_datalayer_member_pii_choice OR \IPS\Member::loggedIn()->datalayer_collect_pii ) )
		{
