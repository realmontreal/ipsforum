...
class _support extends \IPS\Dispatcher\Controller
{
	...
	public function clearLogs() {
		...
		$form = new \IPS\Helpers\Form( 'form', 'custom_clear_logs' );
		...
		$form->add( new \IPS\Helpers\Form\CheckboxSet( 'custom_clear_logs', '', TRUE, array( 'options' => $options, 'multiple' => TRUE, 'impliedUnlimited' => TRUE ) ) );

		if ( $values = $form->values() )
		{
			foreach ($values['custom_clear_logs'] as $key => $value) {
		...
		\IPS\Output::i()->title		= \IPS\Member::loggedIn()->language()->addToStack('__custom_null_alert');
		\IPS\Output::i()->output 	= \IPS\Theme::i()->getTemplate('global')->block( '__custom_null_alert', $form, FALSE );
	}
	...
	protected function customClearCaches()
	{
		/* Check CSRF Key*/
		\IPS\Session::i()->csrfCheck();
		...
		{
			\IPS\Output::i()->redirect( \IPS\Http\Url::internal( "app=core&module={$md}&controller={$ct}" ), 'custom_clear_caches_done' );
		}
	}
