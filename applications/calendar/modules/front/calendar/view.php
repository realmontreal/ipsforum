...
class _view extends \IPS\Dispatcher\Controller
{
	...
	protected function _viewOverview()
	{
		...
		$stream = \IPS\Theme::i()->getTemplate( 'overview', 'calendar' )->byMonth( $calendars, $date, $featured, $events, NULL, $months );
		...
		else
		{
			...
			\IPS\Output::i()->output	= \IPS\Theme::i()->getTemplate( 'overview' )->wrapper( $featured, $nearme, $stream, $form, $mapMarkers, $online, $date );
		}
	...
	protected function search()
	{
		...
		if( $values = $form->values() )
		{
			...
			if( \IPS\GeoLocation::enabled() and \IPS\Request::i()->location )
			{
				...
				else if ( isset( \IPS\Request::i()->lat ) and isset( \IPS\Request::i()->lon ) )
				{
					if( \IPS\Request::i()->lat !== 0 && \IPS\Request::i()->lon !== 0 )
					{
						$location = array( 'lat' => \IPS\Request::i()->lat, 'lon' => \IPS\Request::i()->lon );
					}
					else
			...
			if( isset( \IPS\Request::i()->sortBy ) && \IPS\Request::i()->sortBy == 'nearest' && $location )
			{
				$sort = 'distance asc';
			}
		...
		if( \IPS\Request::i()->isAjax() ){
			...
			$countQuery = \IPS\Db::i()->select( $select, 'calendar_events', $where, NULL, array(), NULL, implode( " OR ", $having ) );
	...
	public static function geocodeLocation( $input = NULL, $asJson = TRUE )
	{
		...
		try
		{
			$data = \IPS\Http\Url::external( "https://api.mapbox.com/geocoding/v5/mapbox.places/" . urlencode( html_entity_decode( $input ) ) . ".json" )->setQueryString( array(
