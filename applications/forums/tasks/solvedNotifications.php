...
class _solvedNotifications extends \IPS\Task
{
	...
	public function execute()
	{
		...
		$where = [
			...
			[ \IPS\Db::i()->in( 'forum_id', iterator_to_array( \IPS\Db::i()->select( 'id', 'forums_forums', '(' . \IPS\Db::i()->bitwiseWhere( \IPS\forums\Forum::$bitOptions['forums_bitoptions'], 'bw_enable_answers_member' ) . ')' ) ) ) ],
			...
			foreach( $topics as $row )
			{
				...
				else
				{
					...
					try
					{
						...
						{
							$replies = $topic->comments( 4, 0, 'date', 'asc' );
							$firstPost = array_shift( $replies );
