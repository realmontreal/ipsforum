...
abstract class _Item extends \IPS\Content
{
	...
	public static function formElements( $item=NULL, \IPS\Node\Model $container=NULL )
	{
		...
		/* Future date */
		if ( \in_array( 'IPS\Content\FuturePublishing', class_implements( \get_called_class() ) ) and static::canFuturePublish( NULL, $container ) and isset( static::$databaseColumnMap['future_date'] ) )
		{
		...
		if ( $this instanceof \IPS\Content\FuturePublishing AND isset( $values[ static::$formLangPrefix . 'date' ] ) )
		{
			...
			elseif ( $container AND ! $this->$column )
			{
				if ( $values[ static::$formLangPrefix . 'date' ] instanceof \IPS\DateTime AND $values[ static::$formLangPrefix . 'date' ]->getTimestamp() > time() )
				{
					...
					$dateColumn    = static::$databaseColumnMap['future_date'];
					$this->$dateColumn = $values[ static::$formLangPrefix . 'date' ]->getTimestamp();
					$this->unPublish();
				}
			}
		}
	...
	public function publish( $member=NULL )
	{
		...
		if ( isset( static::$databaseColumnMap['date'] ) )
		{
			$dateField = \is_array( static::$databaseColumnMap['date'] ) ? static::$databaseColumnMap['date'][0] : static::$databaseColumnMap['date'];
			$this->$dateField = time();
		}
