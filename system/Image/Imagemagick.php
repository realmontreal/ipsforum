...
class _Imagemagick extends \IPS\Image
{
	...
	{
		/* If we want to strip exif, we need to process this image regardless of size */
		if( ! \IPS\Settings::i()->imagick_strip_exif )
		{
			return parent::resizeToMax( $maxWidth, $maxHeight, $retainRatio );
		}

		/* If the image is smaller, let's "resize" it keeping the same dimensions, so it runs via the __toString method */
