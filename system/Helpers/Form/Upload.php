...
class _Upload extends FormAbstract
{
					...
					if ( \IPS\Request::i()->chunk == \IPS\Request::i()->chunks - 1 )
					{
						...
						if ( $fileObj->isImage() AND ( $this->options['checkImage'] OR \is_array( $this->options['image'] ) ) )
						{
							try
							{
								...
								if ( $hasBeenResized or $image->hasBeenRotated )
								{
	...
	public function getValue()
	{
		...
		$return = array();
		$tempFiles = iterator_to_array( \IPS\Db::i()->select( 'id,contents,filename,exif,requires_moderation', 'core_files_temp', array( 'upload_key=?', md5( $this->name . session_id() ) ) )->setKeyField('id') );
		...
		else
		{
			...
			$fileObjects = \IPS\File::createFromUploads( $this->options['storageExtension'], $fieldName, $this->options['allowedFileTypes'], $this->options['maxFileSize'], $this->options['totalMaxSize'], 0, function( $contents, $filename, $i ) use ( $options, &$requiresModeration, $canBeModerated )
			{
				...
				if ( \in_array( $ext, \IPS\Image::supportedExtensions() ) )
				{
					...
					$image = NULL;
					$hasBeenResized = FALSE;
					if ( \is_array( $options['image'] ) OR $options['checkImage'] )
					{
						try
						{
