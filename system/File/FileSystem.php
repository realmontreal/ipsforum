...
class _FileSystem extends \IPS\File
{
	...
	public function chunkInit( $filename, $container = '', $obscure = TRUE )
	{
		...
		$return = array( 'temp' => "{$this->configuration['dir']}/{$tempFolder}/chunk-" . md5( uniqid() ) . '.txt', 'final' => "{$folder}/" . $this->filename );
		touch( $return['temp'] );
		return $return;
	}
	 ...
	 *
	 * @param	mixed	$ref		The reference for this upload as returned by chunkInit() 
	 * @return	string			The file's reference which can be passed to \IPS\File::get() as the URL
	 * @throws	\RuntimeException
	 */
	public function chunkFinish( $ref )
	{
